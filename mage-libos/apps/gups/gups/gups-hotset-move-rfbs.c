...
static void print_inst(uint64_t *prev)
{
  uint64_t tot = 0;
  for (int i = 0; i < threads; i++) {
    tot += thread_gups[i];
  }
  printf("Gups: %.10f\n", (1.0 * (abs(tot - *prev))) / (1.0e9));
  tot_updates += abs(tot - *prev);
  *prev = tot;
}

static void timing_embedded(){
  uint64_t tic = -1, prev = 0;
  bool printed1 = false;
  while(!all_done && !stop){
    tic++;
    print_inst(&prev);
